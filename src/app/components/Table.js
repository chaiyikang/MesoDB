					...
					<table className="min-w-full bg-gray-800 rounded-lg">
							...
							<tr>
								{headers.map(header => (
									<th key={header} className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">
										<div onClick={() => handleSort(header)} className="cursor-pointer hover:bg-gray-700 p-1 rounded">
											{header}
											{sortColumn === header && <span>{sortDirection === "asc" ? " ↑" : " ↓"}</span>}
										</div>
										<input
											type="text"
						...
						<tbody className="bg-gray-700 divide-y divide-gray-600">
							{paginatedData.map((row, i) => (
								<tr key={i}>
									{headers.map(header => (
										<td key={header} className="px-6 py-4 text-sm text-gray-200">
											{header === "Item" ? (
